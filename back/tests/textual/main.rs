...
use naga::front::wgsl::{Frontend, Options};
...
fn parse_wgsl(wgsl_source_text: &str) -> Result<naga::Module, Box<dyn ErrorTrait>> {
    let options = Options {
        parse_doc_comments: true,
        capabilities: naga_rust_back::CAPABILITIES,
    };
    let module: naga::Module = Frontend::new_with_options(options).parse(wgsl_source_text)?;
    Ok(module)
}

fn parse_and_validate_wgsl(
    wgsl_source_text: &str,
) -> Result<(naga::Module, naga::valid::ModuleInfo), Box<dyn ErrorTrait>> {
    let module = parse_wgsl(wgsl_source_text)?;
...
fn translate(config: Config, wgsl_source_text: &str) -> String {
    fn inner(config: Config, wgsl_source_text: &str) -> Result<String, Box<dyn ErrorTrait>> {
        let (module, module_info) = parse_and_validate_wgsl(wgsl_source_text)?;
...
fn expect_error(config: Config, wgsl_source_text: &str) -> naga_rust_back::Error {
    let module: naga::Module = Frontend::new().parse(wgsl_source_text).unwrap();
    let module_info: naga::valid::ModuleInfo = naga::valid::Validator::new(
        naga::valid::ValidationFlags::all(),
        naga_rust_back::CAPABILITIES,
    )
